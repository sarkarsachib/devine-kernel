...
impl Process {
    pub fn new(id: ProcessId, name: String, address_space: AddressSpace) -> Self {
        Self {
            id,
            name,
            address_space,
...
pub fn create_process(name: String, page_table_frame: Frame) -> Option<ProcessId> {
    let mut table = PROCESS_TABLE.lock();
    let pid = table.allocate_pid();
    let address_space = AddressSpace::new(page_table_frame);
    let process = Process::new(pid, name, address_space);
